...
(function( root ) {
    ...
    var ClockTimer = function ( options ) {
        ...
        if (!this.settings.startDate && this.settings.endDate) {
            ...
            //Start of hacky pastiche
            this.settings.total = hours*3600+minutes*60+seconds;//( Math.floor( ( this.settings.endDate - this.settings.startDate ) / 86400 ) );
            this.settings.days = 0;//( Math.floor( ( this.settings.endDate - this.settings.now ) / 86400 ) ); <- TODO: This is the worse of them
            // TODO fix correctly, hacky stopwatch fix
            this.settings.hours = hours; //( 24 - Math.floor( ( ( this.settings.endDate - this.settings.now ) % 86400) / 3600 ) );
            this.settings.minutes = minutes; //( 60 - Math.floor( ( ( ( this.settings.endDate - this.settings.now ) % 86400 ) % 3600) / 60 ) );
            this.settings.seconds = seconds; //( Math.floor( this.settings.now % 60 ) );
        ...
        */
        ...
        this.runHours = function() {
            ...
            if ( _this.canvasHours && this.settings.hoursCanvas === true ) {
                ...
                this.arcArgs = _this.getDeg( 15 * (24 - _this.settings.hours) );

                _this.circle.call( this, _this.canvasHours, this.strokeStyle, this.arcArgs, this.fillStyle );

            }

            this.textValue = _this.settings.hours; // Countdown optim
        ...
        this.runMinutes = function() {
            ...
            if ( _this.canvasMinutes && this.settings.minutesCanvas === true ) {
                ...
                this.arcArgs = _this.getDeg( 6 * (60 - _this.settings.minutes) );
        ...
        this.runSeconds = function() {
            ...
            if ( _this.canvasSeconds && this.settings.secondsCanvas === true ) {
                ...
                this.arcArgs = _this.getDeg( 6 * (60 - _this.settings.seconds) );
        ...
        this.start = function() {
            ...
            if ( _this.settings.now >= _this.settings.endDate ) {
                __this.emit("end", {didSomething: false});
                return;
            }
            */
            ...
            this.run = function(){
                ...
                if ( _this.settings.seconds === 0 ) {
                    ...
                    if (_this.settings.minutes === 0 && _this.settings.hours === 0) {

                        // Countdown complete

                        clearInterval(this.interval);

                        me.emit("end", "I'm breaking out");

                        return;
                    }

                    _this.settings.seconds = 59;

                    if ( _this.settings.minutes === 0 ) {


                        _this.settings.minutes = 59;
    ...
    util.inherits(ClockTimer, emitter) // http://www.tutorialsteacher.com/nodejs/nodejs-eventemitter
